...
if [ -z "$1" ]; then
    kubectl apply -f $SCRIPT_DIR/supplier-helidon-se-deployment.yaml -n msdataworkshop
else
    kubectl apply -f <(istioctl kube-inject -f $SCRIPT_DIR/supplier-helidon-se-deployment.yaml ) -n msdataworkshop
fi
